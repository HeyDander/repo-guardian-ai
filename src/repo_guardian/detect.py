...
def detect(repo: Repository) -> list[str]:
    names = {p.name for p in repo.files()}
    suffixes = {p.suffix for p in repo.files()}
    result: list[str] = []
    if "package.json" in names:
        result += ["JavaScript", "Node.js"]
        package = repo.read(repo.root / "package.json")
        if '"typescript"' in package or ".ts" in " ".join(str(p) for p in repo.files()): result.append("TypeScript")
    ...
    if any("fastapi" in repo.read(p).lower() for p in repo.files() if p.name in {"requirements.txt", "pyproject.toml"}): result.append("FastAPI")
    ...
    if "Dockerfile" in names or any(p.name.startswith("docker-compose") for p in repo.files()): result.append("Docker")
    if any(p.suffix in {".tf"} for p in repo.files()): result.append("Terraform")
